...
my_matrix.file <- file.path('tutorial_run', '1_preprocessing_output', 'matrix_features.csv')
my_report.file <- file.path('tutorial_run', '1_preprocessing_output', 'Report_processed_MolecFormulas.csv')
...
my_outdir <- file.path('tutorial_run', '5_statistics')
...
my_colors <- get_palette('Dark2', length(unique(metadata$Treatment)))
names(my_colors) <- unique(metadata$Treatment) 
...
permanova <- adonis2(dist.matrix ~ Treatment + NULL, 
                    data=metadata, 
                    permutations=999, 
                    method=dm.method)
