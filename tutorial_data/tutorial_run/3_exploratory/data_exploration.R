...
my_data.file <- file.path('tutorial_run', '1_preprocessing_output', 'Report_processed_MolecFormulas.csv')
my_metadata.file <- file.path('metadata.csv')
my_elcomp.file <- file.path('tutorial_run', '1_preprocessing_output', 'elemental_composition.csv')
my_classcomp.file <- file.path('tutorial_run', '1_preprocessing_output', 'class_composition.csv')
classification.file <- file.path('C:/ProgramData/Anaconda3/Lib/site-packages/metabodirect', 'data', 'compound_class_table.csv')
my_outdir <- file.path('tutorial_run', '3_exploratory')
...
my_colors <- get_palette('Dark2', length(unique(df_longer$Treatment)))
names(my_colors) <- unique(df_longer$Treatment) 
...
for(var in c('GFE', 'AI_mod', 'DBE')){
  ...
  var_s = syms(var)
  formula <- formula(paste0(var, ' ~ Treatment'))
  stat.test <- df_longer %>% 
    group_by(NULL) %>% 
    tukey_hsd(formula) %>% 
...
el_bar <- el_comp %>% 
  group_by(Treatment, NULL, Element)  %>% 
  summarise(Count = mean(Count, na.rm = TRUE)) %>% 
  mutate(Perc_count = Count/sum(Count, na.rm = TRUE)*100) %>%
  ggplot(aes(x = Treatment,
             y = Perc_count,
...
for(val in unique(df_longer$'Treatment')){
  list_mass <- df_longer %>%
    filter((!!! group1_s) == val) %>% 
    pull(Mass)
...
names(group_list) <- unique(df_longer$'Treatment')
...
comb_g1 <- combn(unique(df_longer$'Treatment'), 2)

my_colors <- c(get_palette('Dark2', length(unique(df_longer$'Treatment'))), 'gray80')
color_names <- c(unique(as.character(df_longer$'Treatment')), 'Shared')
names(my_colors) <- color_names
...
dir.create(comparison_dir)
  ...
  for(val in unique(df_longer$'NULL')){
    list_mass <- df_longer %>%
      filter((!!! group2_s) == val) %>% 
      pull(Mass)
    group_list[i] <- list(list_mass)
  ...
  names(group_list) <- unique(df_longer$'NULL')
  ...
  comb_g2 <- combn(unique(df_longer$'NULL'), 2)
  
  my_colors <- c(get_palette('d3', length(unique(df_longer$'NULL'))), 'gray80')
  color_names <- c(unique(as.character(df_longer$'NULL')), 'Shared')
  names(my_colors) <- color_names
